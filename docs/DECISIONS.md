...
## D16：单机生产优先恢复证明和有界资源，不引入分布式依赖

状态：已采用，连续稳定性证据收集中。

当前目标是一台可信主机和单 GPU，不以 Redis、Kubernetes 或双节点假装高可用。
ModelPort PostgreSQL 每日生成包含配置与 provenance 的完整备份，每周恢复到隔离临时
数据库；Docker 日志固定轮转，日报/运行台检查磁盘、备份新鲜度和 systemd 失败标记。
72 小时门禁用于灰度晋级，168 小时用于建立单机稳定基线；容器 recreate 会重新计时。

本机归档不能覆盖整机/磁盘损坏，仍需用户配置加密异机复制。需要固定 SLA、零停机
升级或公网多租户时，才进入多节点数据库、分布式限流和第二推理节点设计。

## D17：规划、候选和端口边界必须结构化 fail-closed

状态：已采用。

自动推荐只按最大单卡容量选择，并把当前空闲显存作为部署准入条件；多 GPU、忙碌或
共享 GPU 只生成审查信息，不生成有副作用的下一步命令。Catalog 的 `validated`
表示存在可复查硬件档位，`evidenceStatus` 只描述档位匹配，不能替代当前主机的
acceptance 证据。

候选 Profile 必须在加载生产模型选择后显式覆盖 Compose project、容器、端口和
Cache，CI 对最终 Compose JSON 做身份断言。宿主机发布地址在 Compose 中固定为
`127.0.0.1`，Profile 只能更换端口。所有会重新加载权重的显式启动/重启路径先执行
Catalog 完整性校验。

## D18：本机验收状态必须来自可失效的版本化凭证

状态：已采用。

`quick` 不再只证明 API 能返回文本，而是先验证活动模型制品，再原子写入权限 `0600`
的 schema v3 证据。证据绑定应用域机器指纹、GPU 名称/数量/显存、驱动、RAM、
模型/GGUF revision、权重字节数/SHA256、容器镜像，以及 Compose/Catalog/manifest
与 quick 所有传递脚本哈希，并加入正文自校验和 30 天有效期。只保存 machine-id
的应用域 SHA256，不保存原始 machine-id、hostname、Prompt、回复、工具参数、
凭证或原始环境。

只读规划使用无符号链接安全读取，并要求当前用户所有、`0600`、单硬链接、时间合法。
只有全部绑定字段仍匹配时才使用 `validated-on-this-host`。跨主机复制、过期、
驱动、制品、镜像、控制器或测试脚本变化都会自动让旧凭证失效；硬件档位匹配仍单独
保留，不能替代验收。
