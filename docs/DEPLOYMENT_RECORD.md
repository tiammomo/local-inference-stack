...
312 个 Rust lib 测试、Mock 严格 Schema 拒绝和真实 Qwen standard 验收。Dashboard
...
同日安全复核进一步将持久化错误改为类别化记录，并对旧 control/ledger 错误执行
一次性脱敏迁移。真实重建曾验证 `config.toml=0600` 会阻止容器内非 root 用户读取；
该无密钥只读配置最终保持 `0644`，而 `.env`、旧环境备份和运营密钥继续保持
`0600`。ModelPort 镜像、最小权限和回环端口现已进入自动漂移检查。
