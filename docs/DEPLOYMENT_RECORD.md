...
| ModelPort 本地镜像 ID | `sha256:f4c956b8715a104d99d8f4b6a34c8494e0b1c4d48016d5faf560d580be5d8c73` |
...
| ModelPort Tool Use | 通过 | 严格模式下非流式、完整参数流式、`is_error` 与 continuation 均 PASS；mock 拒绝未声明工具和非对象参数 |
...
| ModelPort 基线仓库检查 | 通过 | Docker 内 293 个 lib + 6 个 integration，共 299 个 Rust 测试通过；格式与 diff 检查通过 |
...
本轮 reasoning/sampling/token-counting/Tool Use adapter 改动已通过 Docker release
编译，新增 3 个上下文预算单测和 1 个 Tool Outcome 分类单测通过；此前 Docker
全量基线为 299 个 Rust 测试。Dashboard typecheck、lint、88 个测试和生产构建也已
通过。真实 Anthropic
