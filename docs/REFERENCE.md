...
默认 quick 仅作服务门禁；显式 full 将事务绑定 qualification 放在 rollback pointer 发布前，仍不会自动晋级 Catalog。

- 状态：默认只读；--yes 在单 runtime 维护窗口中下载、停止、替换并执行所选门禁。
- 批准：执行时必须 --yes，source/target/host/attestation 均须准入。
- 语法：`./stack upgrade --model CATALOG_ID [--qualification {quick,full}] [--yes] [--json]`
- 示例：`MODELPORT_PROJECT_DIR=/absolute/path/to/ModelPort ./stack upgrade --model CATALOG_ID --qualification full --json`
