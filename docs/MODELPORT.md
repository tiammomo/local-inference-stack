...
非流式参数违反 Schema 时，ModelPort 最多在同一 Provider 上追加一次独立修复尝试，
失败候选不会交付，修复提示不含参数、校验路径或业务正文，两次调用分别入账并在请求
级合并 Token。流式、超时、认证失败和工具执行错误均不自动重试。日志仅增加
`toolRepairAttempted/toolRepairRecovered` 聚合布尔值。
...
本项目验收请求使用 `x-modelport-traffic-class: synthetic`，运行台默认排除；业务默认
为 `business`，人工排障可使用 `diagnostic`。
