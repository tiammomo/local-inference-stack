...
单 GPU 自动推荐选择最大单卡满足全部最低条件的最大候选，同时要求部署时空闲显存
达到最低门槛。它是可解释的静态规则，不联网，也不声称预测实际 tokens/s 或质量。
硬件档位匹配只代表可复用已记录参数起点，不代表当前主机已通过验收。
...
- **多 GPU**：工具会显示逐卡与总显存，但按最大单卡推荐并强制
  `readyToDeploy=false`。必须审查每卡容量、PCIe/NVLink、tensor split 和最慢卡瓶颈，
  再设计独立 Profile。
- **CPU-only / Apple Silicon / AMD**：Catalog 的 GGUF 仍可能有用，但本 Compose
  明确是 NVIDIA CUDA，不能直接套用。应新增后端 Profile 和独立验收。
- **共享 GPU**：规划会报告空闲显存；低于门槛时不提供部署命令。生产共享还必须禁止
  自动重启抢占资源并使用人工审核 Profile。
