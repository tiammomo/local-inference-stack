...
客户端取消与服务失败分开统计，SLA 使用服务可用率；TTFT 与完整生命周期延迟也分开，
避免用户主动取消或长输出污染首字延迟判断。
...
- 最近时间窗的成功率、延迟、Token、Tool Use 工作流成功率、终态和脱敏错误类别。
...
| Tool Use 失败率 | 有 Tool Use 样本且 `>=5%` | 检查严格响应错误，运行 Tool Use 专项验收 |
...
1. 比较连续快照的请求量、成功率、P50/P95/P99、输出 Token 和 Tool Use 成功率。
