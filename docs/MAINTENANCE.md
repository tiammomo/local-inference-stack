...
客户端取消与服务失败分开统计，SLA 使用服务可用率。当前
`firstByteLatencyMs` 在流式与非流式路径的语义不同，只能作为响应开始/缓冲耗时观察；
真正的流式 TTFT 完成后才能与完整生命周期延迟做严格对照。
...
临时 `local_tool_acceptance_*` mock provider 默认不进入生产成功率和告警分母，但
报告会给出排除数量；需要审计测试流量时加 `--include-synthetic`。真实 Qwen Tool
Use 验收仍经过 `local_qwen`，因此继续计入。

ModelPort 部署新字段前的旧请求按 `toolOutcome=unknown_legacy` 兼容读取，因此 Tool
Use 结果趋势应从本次部署时间开始计算；历史工作流可以进入请求分母，但不能误标为
“未请求工具”。

当前 Tool Use 成功分子仍来自 API 请求成功，不表示工具已执行或最终答案正确。巡检
时应称为“Tool Use 请求成功率”；闭环状态上线后再同时观察 Schema、续接和任务成功率。
本项目通过 `local_qwen` 运行的真实上游验收暂时仍计入窗口，分析业务趋势时需要结合
部署和验收时间判断，后续以显式 traffic class 排除。
