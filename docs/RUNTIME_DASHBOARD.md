...
- Tool Use 请求成功率、模型/Provider 分布、请求终态和活动告警；
...
响应开始时间、缓存命中、Tool Use 请求成功率和资源指标，不含任何调用正文或标识符。

## 当前指标口径与待升级项

- `Tool Use 成功率` 当前表示“请求携带 Tools 且 API 成功结束”，不是工具执行或最终
  任务完成率；历史 `unknown_legacy` 也不能解释为失败。
- `firstByteLatencyMs` 在流式路径接近上游响应建立时间，在非流式路径等于完整响应
  耗时，因此当前页面不得将混合分位命名为严格 TTFT。
- 通过 `local_qwen` 运行的真实上游验收目前仍进入业务窗口；临时 Mock Provider
  已排除，但下一阶段还需要显式 traffic class。

Tool Use Reliability v2 将分别展示请求可用性、模型调用率、Schema 通过率、
Tool Result 续接率和闭环任务成功率；TTFT 将只对流式首个可交付语义事件计算。
完整定义见 [`ENHANCEMENT_ROADMAP.md`](ENHANCEMENT_ROADMAP.md)。
